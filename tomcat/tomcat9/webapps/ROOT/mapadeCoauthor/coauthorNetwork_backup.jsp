...
<%-- =====================================================================
  ...
  ESTRUCTURA DEL CODIGO (FLUJO DE EJECUCION):
    ...
    3. TRIGGER BACKGROUND: Lanza el metodo 'launchRebuild' en un Hilo separado (Thread).
       ...
       - I/O: Se crea el JSONObject giganta y se sobreescribe fÃ­sicamente 'baseData.json'.
  ===================================================================== --%>
        ...
        Thread t = new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    // Cuenta publicaciones y grants por separado.
                    // Obtiene la facultad/escuela a traves de la posicion.
                    // ---------------------------------------------------------
                    String Q_INTERNAL =
                        ...
                        // Filtro de tipos de publicacion validos
                        +"  VALUES ?publi { bibo:AcademicArticle bibo:Book bibo:Chapter fabio:Comment "
                        +"vivo:EditorialArticle vivo:Grant vivo:WorkingPaper vivo:Abstract "
                        +"vivo:ConferencePaper vivo:ConferencePoster bibo:Letter bibo:Report "
                        +"vivo:Review obo:ERO_0000071 vivo:Speech bibo:Thesis vivo:Video }\n"
                        +"  ?doc a ?publi.\n"
                        // Identificar grants para contarlos aparte
                        +"  OPTIONAL { ?doc a vivo:Grant. BIND(?doc AS ?grantDoc) }\n"
                        // Obtener facultad/escuela del investigador
                        +"  OPTIONAL {\n"
                        +"    VALUES ?posType { vivo:FacultyAdministrativePosition vivo:FacultyPosition "
                        ...
                        +"  }\n"
                        // Extraer slug del URI como ID (ej: "ricardo-abello-galvis")
                    ...
                    // Se identifican como foaf:Person que NO son vlocal:UREntity.
                    // Todos se marcan con group='external'.
                    // ---------------------------------------------------------
                    String Q_EXTERNAL =
                        "PREFIX vivo: <http://vivoweb.org/ontology/core#>\n"
                        +"PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\n"
                        ...
                        +"(COUNT(DISTINCT ?doc) AS ?totalDocs) (0 AS ?grants)\n"
                        +"WHERE {\n"
                        +"  ?person a foaf:Person; rdfs:label ?name; vivo:relatedBy ?NA.\n"
                        +"  ?NA a vivo:Authorship; vivo:relates ?doc.\n"
                        // Excluir investigadores internos de la UR
                    ...
                    // Cuenta publicaciones y grants conjuntos.
                    ...
                    String Q_EDGES_INT =
                        ...
                        +"PREFIX fabio: <http://purl.org/spar/fabio/>\n"
                        +"PREFIX obo: <http://purl.obolibrary.org/obo/>\n"
                        +"SELECT ?sID ?tID (COUNT(DISTINCT ?doc) AS ?jointTotalDocs) "
                        +"(COUNT(DISTINCT ?grantDoc) AS ?jointGrants)\n"
                        +"WHERE {\n"
                        +"  VALUES ?publi { bibo:AcademicArticle bibo:Book bibo:Chapter fabio:Comment "
                        +"vivo:EditorialArticle vivo:Grant vivo:WorkingPaper vivo:Abstract "
                        +"vivo:ConferencePaper vivo:ConferencePoster bibo:Letter bibo:Report "
                        ...
                        +"  ?doc a ?publi; vivo:relatedBy ?NA2.\n"
                        ...
                        // Orden alfabetico para evitar duplicados
                    ...
                    String Q_EDGES_EXT =
                        ...
                        +"  ?p2 a foaf:Person.\n"
                        +"  FILTER(?p1 != ?p2)\n"
                        // Solo aristas con personas externas (no UR)
                    ...
                    for (int pass = 0; pass < 2; pass++) {
                        ...
                        try {
                            ...
                            for (int i = 0; i < rows.length(); i++) {
                                ...
                                // Restar grants del total para obtener solo publicaciones
...
<%
    ...
    String realPath = application.getRealPath("/mapadeCoauthor/") + "baseData.json";
