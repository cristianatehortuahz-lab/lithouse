...
function initNetwork(data) {
    ...
    // =========================================================
    // 0.5 FILTRADO DE OPTIMIZACIÓN
    // Eliminar nodos sin conexiones para evitar que D3 se congele
    // (Aplica especialmente para autores externos sin jointPubs/Grants)
    // =========================================================
    const validNodeSet = new Set();
    data.edgesInternal.forEach(e => { validNodeSet.add(e.source); validNodeSet.add(e.target); });
    data.edgesAll.forEach(e => { validNodeSet.add(e.source); validNodeSet.add(e.target); });
    
    // Filtramos los nodos. Mantenemos también a los investigadores internos inactivos 
    // solo si se desea, pero por rendimiento es mejor dibujar solo los que tienen aristas.
    data.nodesInternal = data.nodesInternal.filter(n => validNodeSet.has(n.id) || !n.external);
    data.nodesAll = data.nodesAll.filter(n => validNodeSet.has(n.id) || !n.external);
