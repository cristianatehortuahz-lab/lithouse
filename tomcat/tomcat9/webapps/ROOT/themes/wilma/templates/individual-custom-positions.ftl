...
<#if positions?has_content> <#-- true when the property is in the list, even if not populated (when editing) -->
   ...
   <#--  <h4 id="${localName}" class="mainPropGroup" title="${positions.publicDescription!}">
   		${positions.name?capitalize} <@p.addLink positions editable /> <@p.verboseDisplay positions />
   	</h4>  -->
    <#if user.hasSiteAdminAccess>
        <h4>${positions.name?capitalize} <@p.addLink positions editable /> <@p.verboseDisplay positions /></h4>
    </#if>
    <ul id="individual-personInPosition" class="position-list" role="list">
