...
<#else>
    ...
    <#local resourceTitle>
        <#if statement.infoResource??>
             <#if statement.doi??>
                    <a href="https://doi.org/${statement.doi}"  title="${i18n().resource_name}" target="_blank">${statement.infoResourceName}</a>
                <#elseif statement.pmid??>
                    <a href="https://pubmed.ncbi.nlm.nih.gov/${statement.pmid}"  title="${i18n().resource_name}" target="_blank">${statement.infoResourceName}</a>
                <#else>
    ...
    <span class="row-title">${resourceTitle}</span> <#--  ${citationDetails}  --> <div class="row_footer d-flex"><@dt.yearSpan "${statement.dateTime!}" /> ${altMetric} ${plum}</div>
</#if>
</#macro>
