...
<#macro dataPropertyList property editable template=property.template>
    <#list property.statements as statement>
        <@propertyListItem property statement editable ><#-- <span>"data ${template}"</span> --><#include "${template}"></@propertyListItem>
    </#list>
</#macro>
...
<#--  <#macro objectPropertyList property editable statements=property.statements template=property.template>
    ...
    <#list statements as statement>        
        ...
        <h3>Preguntar si es tesis ${template} ${property.localName}</h3>
    </#list>
</#macro>  -->
<#--  <@propertyListItem property statement editable><#include "propStatement-custom-person.ftl"></@propertyListItem>  -->

<#--  Ejemplo para inyectar shortview  -->
<#macro objectPropertyList property editable statements=property.statements template=property.template>
    <#if statements?? && statements?has_content>
        <#list statements as statement>
            <@propertyListItem property statement editable><#include "${template}"></@propertyListItem>
        </#list>
    <#else>
...
<#macro image individual propertyGroups namespaces editable showPlaceholder="never" imageWidth=160 >
    ...
    <#if (mainImage.statements)?has_content && thumbUrl?has_content>
        <a href="${individual.imageUrl}" title="${i18n().alt_thumbnail_photo}">
        	<img loading="lazy" class="individual-photo" src="${thumbUrl}" title="${i18n().click_to_view_larger}" alt="${individual.name}" width="${imageWidth!}" />
        </a>
    ...
    <#else>
        ...
        <#if showPlaceholder == "always" || (showPlaceholder="with_add_link" && imageLabel?has_content)>
            <img loading="lazy" class="individual-photo" src="${placeholderImageUrl(individual.uri)}" title = "${i18n().no_image}" alt="${i18n().placeholder_image}" width="${imageWidth!}" />
        </#if>
    </#if>
</#macro>
